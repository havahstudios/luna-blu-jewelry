...
async function migrate() {
  console.log(`\nMigrating ${CATALOG.length} products to Sanity...\n`);
  let success = 0, failed = 0;

  for (const product of CATALOG) {
    process.stdout.write(`[${success + failed + 1}/${CATALOG.length}] ${product.name}...`);

    try {
      const imageRef = product.img ? await uploadImage(product.img, product.slug) : null;
